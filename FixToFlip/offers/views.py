...
def add_offer_view(request, pk):
    if (
        request.method == "POST"
        and not Offer.objects.filter(listed_property__pk=pk).exists()
    ):
...
class EditOfferView(LoginRequiredMixin, UserPassesTestMixin, UpdateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        property_form = PropertyOfferEditForm(
            request.POST, instance=self.object.listed_property
        )
        ...
        if user_profile.profile_type == "Personal":
            if (
                ...
                and user_profile.user.last_name
                and user_profile.phone_number
            ):
                pass
            else:
                property_form.add_error(
                    None,
                    "As a personal profile, please provide your first name, last name, "
            ...
            if user_profile.company_name and user_profile.company_phone:
                ...
                property_form.add_error(
                    None,
                    "As a company profile, you must provide a company name and a phone "
                    "number before publishing a listing",
                )
        elif not user_profile.profile_type:
            property_form.add_error(
                None,
                f"Please set up your profile type and provide the necessary details before "
        ...
        if offer_form.is_valid() and property_form.is_valid():
            ...
            if offer_form.instance.offer_status == "Sold":
                property_form.instance.property_condition = (
                    choices.PropertyConditionChoices.SOLD
                )
            ...
            property_form.instance.is_published = True
            property_form.save()
