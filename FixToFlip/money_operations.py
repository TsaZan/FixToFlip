...
def sum_current_expenses(property_id):
    expenses = (
        PropertyExpense.objects.filter(property_id=property_id)
        .values(
            "utilities",
            "notary_taxes",
            "profit_tax",
...
def credit_balance(credit_id):
    ...
    total_principal_amount = CreditPayment.objects.filter(
        credit_id=credit_id
    ).aggregate(Sum("principal_amount"))["principal_amount__sum"]
    ...
    if not isinstance(total_principal_amount, Money):
        total_principal_amount = Money(
            total_principal_amount, credit.credit_amount.currency
        )
