...
def create_expense_notes(sender, instance, created, **kwargs):
    if created:
        credited_properties = PropertyFinancialInformation.objects.filter(
            credit_id=instance.credit_id, credited_amount__gt=0
        )
        total_credit_amount = credited_properties.aggregate(Sum("credited_amount"))

        for property in credited_properties:
            credit_share = (
                property.credited_amount.amount
                / total_credit_amount["credited_amount__sum"]
            )
            ...
            PropertyExpense.objects.filter(
                id=property.property.property_expenses.first().id
            ).update(
                credit_interest=property.property.property_expenses.first().credit_interest.amount
...
def delete_related_expense_notes(sender, instance, **kwargs):
    ...
    for property in credited_properties:
        ...
        if related_expense:
            ...
            if related_expense.credit_interest < Money(
                0, related_expense.credit_interest.currency
            ):
                related_expense.credit_interest = Money(
                    0, related_expense.credit_interest.currency
                )
