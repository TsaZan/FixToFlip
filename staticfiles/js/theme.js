...
// ------------------------------------- Chart
        $(document).ready(function() {
            ...
            if (chartP.length) {
                var propertyStatusChart = new Chart(chartP, {
                    type: 'doughnut',
                    data: {
                        labels: [],
                        datasets: [{
                ...
                fetch('/properties/api-condition/')
                    .then(response => response.json())
                    .then(data => {
                        // Process the data and update the chart
                        const labels = Object.keys(data); // The statuses
                        const values = Object.values(data); // The counts of properties

                        // Update the chart's labels and data dynamically
                        propertyStatusChart.data.labels = labels;
                        propertyStatusChart.data.datasets[0].data = values;
        ...
        fetch('/properties/api-expenses/')  // Full path to the API
            ...
            .then(data => {
                ...
                var ctx = document.getElementById('property-expenses-chart').getContext('2d');
                var propertyExpensesChart = new Chart(ctx, {
                    type: 'bar',
                    data: {
                        labels: [
                            ...
                            'Electrical Repair Expenses', 'Roof Repair Expenses', 'Facade Repair Expenses', 'Other Repair Expenses'
